(function () {
  ...
  function filteredCards() {
    ...
    return cards.filter(function (card) {
      ...
      var searchOk = !search || (card.word + " " + card.meaning).toLowerCase().indexOf(search) > -1;
  ...
  function renderBrowse() {
    ...
    list.slice(0, 300).forEach(function (card) {
      ...
      btn.innerHTML = "<small>" + escapeHtml(card.section) + " / " + escapeHtml(card.set) + " / " + escapeHtml(labelStatus(item.status)) + "</small><strong>" + escapeHtml(card.word) + "</strong><p>" + escapeHtml(card.meaning) + "</p>";
      btn.addEventListener("click", function () {
        queue = [card];
        currentIndex = 0;
        els.studyView.hidden = false;
